...
function isOperationalObject(object: KnowledgeObject) {
  if (["TASK", "NANOBOT_MISSION", "NANOBOT_REPORT"].includes(object.objectType)) return true;
  const searchable = `${object.title} ${object.description} ${object.content} ${object.tags.join(" ")}`.toLowerCase();
  return object.sourceType === "AI_INFERENCE" && /software worker|worker handoff|mission handoff|handoff/.test(searchable);
}
...
export default function KnowledgeSpace() {
  ...
  const [workspaceView, setWorkspaceView] = useState<"knowledge" | "attention" | "research" | "datasets">("knowledge");
  ...
  const operationalObjects = useMemo(() => objects.filter((object) => object.status !== "TRASHED" && isOperationalObject(object)), [objects]);
  const visibleUnfiled = useMemo(() => unfiled.filter((object) => !isOperationalObject(object)), [unfiled]);
  const workspaceItems = useMemo(() => {
    const activeObjects = objects.filter((object) => object.status !== "TRASHED" && !isOperationalObject(object) && object.objectType !== "FOLDER");
    if (workspaceView === "attention") return activeObjects.filter((object) => object.status === "NEEDS_ATTENTION" || ["REQUIRES_REVIEW", "CONTRADICTED", "UNMAPPED", "NOT_ESTABLISHED"].includes(object.truthState));
    if (workspaceView === "research") return activeObjects.filter((object) => ["RESEARCH_QUESTION", "HYPOTHESIS", "OBSERVATION", "NOTE", "DOCUMENT"].includes(object.objectType));
    if (workspaceView === "datasets") return activeObjects.filter((object) => ["DATASET", "SCIENTIFIC_STRUCTURE", "SCIENTIFIC_REGION", "EVIDENCE_RECORD", "REFERENCE"].includes(object.objectType));
    return visibleUnfiled;
  }, [objects, visibleUnfiled, workspaceView]);
  ...
  const childrenOf = (folderId: string) => (itemsByFolder.get(folderId) ?? []).filter((item) => item.id !== folderId && !isOperationalObject(item)).slice(0, 80);
  ...
  return (
    <div className="senota-page space-y-4 pb-4">
      ...
      {activePanel === "workspace" ? (
        <div className="grid min-h-[640px] gap-3 xl:grid-cols-[260px_minmax(0,1fr)_300px]">
          <aside className="rounded-2xl border border-white/10 bg-card/70 p-3">
            <div className="mb-3 flex items-center justify-between px-1"><div><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-500">Knowledge navigation</p><p className="mt-1 text-xs text-slate-400">Folders organize knowledge references without duplicating records.</p></div><Folder className="size-4 text-cyan-300" /></div>
            <div className="mb-3 grid grid-cols-2 gap-1 rounded-xl border border-white/10 bg-slate-950/20 p-1">{([ ["knowledge", "Knowledge"], ["attention", "Needs attention"], ["research", "Research / gaps"], ["datasets", "Datasets"] ] as const).map(([view, label]) => <button key={view} onClick={() => setWorkspaceView(view)} className={`rounded-lg px-2 py-1.5 text-left text-[10px] font-semibold transition ${workspaceView === view ? "bg-cyan-300/10 text-cyan-100" : "text-slate-500 hover:bg-white/5 hover:text-slate-200"}`}>{label}</button>)}</div>
            <div className="space-y-1">
              {workspaceView === "knowledge" ? folders.map((folder) => <FolderBranch key={folder.id} folder={folder} children={childrenOf(folder.id)} selectedId={selectedId} onSelect={setSelectedId} />) : null}
              <div className="mt-3 border-t border-white/10 pt-3"><p className="px-2 pb-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-600">{workspaceView === "knowledge" ? "Unfiled knowledge" : workspaceView === "attention" ? "Requires review" : workspaceView === "research" ? "Questions and working notes" : "Evidence and datasets"} · {workspaceItems.length}</p>{workspaceItems.slice(0, 80).map((object) => <TreeItem key={object.id} object={object} selectedId={selectedId} onSelect={setSelectedId} />)}{!workspaceItems.length ? <p className="px-2 py-3 text-xs leading-5 text-slate-600">No items currently match this view.</p> : null}</div>
            </div>
            {operationalObjects.length ? <button onClick={() => setActivePanel("activity")} className="mt-4 w-full rounded-xl border border-violet-400/15 bg-violet-400/5 p-3 text-left transition hover:bg-violet-400/10"><div className="flex items-center justify-between gap-2"><span className="text-xs font-semibold text-violet-100">Operations & worker handoffs</span><span className="text-xs text-violet-200">{operationalObjects.length}</span></div><p className="mt-1 text-[11px] leading-4 text-slate-500">Preserved separately so operational artifacts do not obscure the knowledge map.</p></button> : null}
            <div className="mt-4 rounded-xl border border-violet-400/15 bg-violet-400/5 p-3"><div className="flex gap-2"><ShieldCheck className="mt-0.5 size-4 shrink-0 text-violet-300" /><p className="text-xs leading-5 text-slate-400">Provider snapshots are immutable. Create notes, links, questions, and proposals around them without overwriting their source evidence.</p></div></div>
          </aside>
      ...
      {activePanel === "activity" ? <ActivityPanel missions={missions} activity={activity} audit={audit.data ?? []} operationalObjects={operationalObjects} onOpenObject={(id) => { setSelectedId(id); setActivePanel("workspace"); }} auditHint="Every persisted create, edit, soft-delete, restore, relationship, autonomy change, and mission transition creates a server-side audit event." /> : null}
