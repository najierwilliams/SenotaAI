...
import { MAX_NPC_CANON_REQUEST_CHARS, MAX_NPC_RUNTIME_EXCERPT_CHARS } from "@shared/const";
import {
  addWorkspaceMemory,
      ...
      <Dialog open={isCanonDialogOpen} onOpenChange={setIsCanonDialogOpen}>
        <DialogContent className="max-h-[90vh] max-w-3xl overflow-y-auto border-violet-300/20 bg-slate-950 text-slate-100 sm:rounded-2xl">
          ...
          {!canonDraft ? <div className="space-y-4 py-2">
            ...
            <div className="space-y-1.5"><label className="block text-xs font-medium text-slate-300">What should be permanent NPC canon?<textarea value={canonRequest} onChange={(event) => setCanonRequest(event.target.value)} maxLength={MAX_NPC_CANON_REQUEST_CHARS} aria-describedby="canon-request-guidance canon-request-count" placeholder="Describe the lore, personality, relationship, place, or dialogue boundary you want added..." className="mt-1.5 min-h-36 w-full resize-y rounded-lg border border-white/10 bg-black/25 p-3 text-sm leading-6 text-white outline-none focus:border-violet-300/60" /></label><p id="canon-request-count" aria-live="polite" className={`text-right text-[11px] ${canonRequest.length > MAX_NPC_CANON_REQUEST_CHARS * 0.9 ? "text-amber-200" : "text-slate-500"}`}>{canonRequest.length.toLocaleString()} / {MAX_NPC_CANON_REQUEST_CHARS.toLocaleString()} characters</p></div>
            <p id="canon-request-guidance" className="rounded-lg border border-amber-300/15 bg-amber-300/[0.04] px-3 py-2 text-xs leading-5 text-amber-100/90">Detailed canon requests up to {MAX_NPC_CANON_REQUEST_CHARS.toLocaleString()} characters are supported. Only permanent NPC lore belongs here; player histories, secrets, tokens, passwords, and raw conversation transcripts are blocked. Keep the final <code>## Runtime excerpt</code> concise: only its first {MAX_NPC_RUNTIME_EXCERPT_CHARS.toLocaleString()} characters are supplied to live dialogue.</p>
