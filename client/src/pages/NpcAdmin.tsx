...
export default function NpcAdmin() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const [canonResponse, memoryResponse, relationshipResponse, auditResponse] = await Promise.all([
        ...
        request(`/api/npc/admin/memories?${queryFilters(npcFilter, playerFilter, includeInactive ? { includeInactive: "true" } : {})}`),
  ...
  return <div className="mx-auto flex w-full max-w-7xl flex-col gap-5"><section className="senota-hero-grid relative overflow-hidden rounded-[1.75rem] border border-cyan-300/15 bg-card/70 px-6 py-7"><div className="relative flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-cyan-300"><Database className="size-4" /> NPC governance plane</div><h1 className="mt-3 font-display text-3xl font-semibold text-white">Canon, relationships, and memory</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">Obsidian remains permanent canon. This secured workspace governs bounded runtime lore, player-specific relationship dimensions, and reversible memory review.</p></div><button onClick={() => void load()} disabled={busy} className="inline-flex w-fit items-center gap-2 rounded-xl border border-cyan-300/20 bg-cyan-300/10 px-4 py-2.5 text-sm text-cyan-100"><RefreshCw className={`size-4 ${busy ? "animate-spin" : ""}`} /> Refresh</button></div></section>
    ...
    <section className="rounded-2xl border border-white/10 bg-card/60 p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-semibold text-white">Player interaction memory review</p><p className="mt-1 text-xs text-slate-500">Pin durable facts, edit summaries, set expirations, deactivate, or restore memories.</p></div><Badge className="border border-violet-300/20 bg-violet-300/10 text-violet-100">{memories.length} records</Badge></div><div className="mt-4 grid gap-2 md:grid-cols-[1fr_1fr_auto_auto]"><input value={npcFilter} onChange={event => setNpcFilter(event.target.value)} className="field" placeholder="Filter by NPC ID" /><input value={playerFilter} onChange={event => setPlayerFilter(event.target.value)} className="field" placeholder="Filter by player UUID" /><label className="flex items-center gap-2 rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-xs text-slate-300"><input type="checkbox" checked={includeInactive} onChange={event => setIncludeInactive(event.target.checked)} /> Include inactive</label><button onClick={() => void load()} className="rounded-lg border border-cyan-300/20 bg-cyan-300/10 px-3 py-2 text-xs font-semibold text-cyan-100">Apply</button></div><div className="mt-4 grid gap-3 lg:grid-cols-2">{memories.length ? memories.map(memory => <article key={memory.id} className={`rounded-xl border p-4 ${memory.isPinned ? "border-amber-300/25 bg-amber-300/[0.045]" : "border-white/10 bg-black/10"}`}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap gap-1.5"><Badge className="border border-white/10 bg-white/5 text-[10px] text-slate-300">{memory.npcId}</Badge><Badge className="border border-cyan-300/15 bg-cyan-300/5 text-[10px] text-cyan-100">{memory.memoryKind}</Badge><Badge className="border border-white/10 bg-white/5 text-[10px] text-slate-400">Importance {memory.importance}</Badge>{memory.isPinned ? <Badge className="border border-amber-300/20 bg-amber-300/10 text-[10px] text-amber-100">Pinned</Badge> : null}</div><p className="mt-2 text-xs leading-5 text-slate-200">{memory.summary}</p><p className="mt-2 text-[10px] text-slate-500">Player {memory.playerId.slice(0, 8)}… · {memory.source}{memory.expiresAt ? ` · Expires ${new Date(memory.expiresAt).toLocaleDateString()}` : " · No expiry"}</p></div><div className="flex shrink-0 gap-1"><button onClick={() => editText("Edit concise interaction summary", memory.summary, next => void updateMemory(memory.id, { summary: next }))} className="icon-button" title="Edit summary"><Pencil className="size-4" /></button><button onClick={() => void updateMemory(memory.id, { isPinned: !memory.isPinned })} className={`icon-button ${memory.isPinned ? "border-amber-300/25 text-amber-100" : ""}`} title={memory.isPinned ? "Unpin memory" : "Pin memory"}>{memory.isPinned ? <PinOff className="size-4" /> : <Pin className="size-4" />}</button><button onClick={() => editExpiry(memory)} className="icon-button" title="Set or clear expiration"><CalendarClock className="size-4" /></button><button onClick={() => void updateMemory(memory.id, { isActive: !memory.isActive })} className={`icon-button ${memory.isActive ? "border-emerald-300/20 text-emerald-200" : ""}`} title={memory.isActive ? "Deactivate memory" : "Restore memory"}>{memory.isActive ? <EyeOff className="size-4" /> : <RotateCcw className="size-4" />}</button></div></div></article>) : <Empty label="No matching player-memory summaries." icon={Users} />}</div></section>
