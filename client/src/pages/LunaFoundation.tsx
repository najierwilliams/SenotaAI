...
export default function LunaFoundation() {
  ...
  const { data: settings, isLoading } = trpc.agent.settings.get.useQuery();
  const { data: connections } = trpc.agent.connections.useQuery();
  const update = trpc.agent.settings.update.useMutation({ onSuccess: () => { void utils.agent.settings.get.invalidate(); } });
  const [form, setForm] = useState({ lunaName: "Luna", lunaStartingAge: 0, lunaNativeLanguage: "English", lunaPersonalityFoundation: "", lunaPersonalityKnowledge: "", lunaAppearanceReference: "" });
  useEffect(() => {
    if (!settings) return;
    setForm({ lunaName: settings.lunaName, lunaStartingAge: settings.lunaStartingAge, lunaNativeLanguage: settings.lunaNativeLanguage, lunaPersonalityFoundation: settings.lunaPersonalityFoundation, lunaPersonalityKnowledge: settings.lunaPersonalityKnowledge, lunaAppearanceReference: settings.lunaAppearanceReference });
  }, [settings]);
  ...
  return <div className="senota-page mx-auto flex w-full max-w-6xl flex-col gap-6">
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_300px]">
      <section className="space-y-6 rounded-[1.5rem] border border-white/10 bg-card/70 p-5 sm:p-7">
        ...
        <div className="grid gap-4 sm:grid-cols-3"><label className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Name<input value={form.lunaName} onChange={e => set("lunaName", e.target.value)} className={fieldClass} disabled={isLoading} /></label><label className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Starting age<input type="number" min={0} max={150} value={form.lunaStartingAge} onChange={e => set("lunaStartingAge", Number(e.target.value))} className={fieldClass} disabled={isLoading} /></label><label className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">Native language<input value={form.lunaNativeLanguage} onChange={e => set("lunaNativeLanguage", e.target.value)} className={fieldClass} disabled={isLoading} /></label></div>
        <div><h2 className="text-lg font-semibold text-white">Personality foundation</h2><p className="mt-1 text-sm text-slate-500">Creator-provided starting principles that Luna can reflect on and develop from.</p><textarea value={form.lunaPersonalityFoundation} onChange={e => set("lunaPersonalityFoundation", e.target.value)} className={areaClass} disabled={isLoading} /></div>
        <div><h2 className="text-lg font-semibold text-white">Personality knowledge</h2><p className="mt-1 text-sm text-slate-500">Initial context and knowledge the cognitive core may use as a starting point.</p><textarea value={form.lunaPersonalityKnowledge} onChange={e => set("lunaPersonalityKnowledge", e.target.value)} className={areaClass} disabled={isLoading} /></div>
        <div><h2 className="text-lg font-semibold text-white">Future appearance reference</h2><p className="mt-1 text-sm text-slate-500">A protected creator reference for future embodiment. Body and clothing systems are not part of this milestone.</p><textarea value={form.lunaAppearanceReference} onChange={e => set("lunaAppearanceReference", e.target.value)} className={areaClass} disabled={isLoading} /></div>
        <div className="flex items-center justify-between gap-3"><p className="text-xs text-slate-500">Changes are saved to the authenticated creator settings record.</p><button onClick={save} disabled={isLoading || update.isPending} className="inline-flex items-center gap-2 rounded-xl bg-cyan-300 px-4 py-2.5 text-sm font-semibold text-slate-950 transition hover:bg-cyan-200 disabled:opacity-50"><Save className="size-4" /> {update.isPending ? "Saving…" : "Save foundation"}</button></div>{update.error ? <p className="text-sm text-rose-300">{update.error.message}</p> : null}
      </section>
      <aside className="space-y-4"><section className="rounded-[1.5rem] border border-white/10 bg-card/70 p-5"><h2 className="text-sm font-semibold text-white">Luna state</h2><div className="mt-4 space-y-3 text-sm"><div className="flex justify-between gap-3"><span className="text-slate-500">Current age</span><span className="font-medium text-cyan-100">{settings?.lunaCurrentAge ?? "—"}</span></div><div className="flex justify-between gap-3"><span className="text-slate-500">Development</span><span className="font-medium text-emerald-200">Evolving from foundation</span></div><div className="flex justify-between gap-3"><span className="text-slate-500">Safety authority</span><span className="font-medium text-emerald-200">Independent</span></div><div className="flex justify-between gap-3"><span className="text-slate-500">Reasoning brain</span><span className={connections?.ollamaConfigured ? "font-medium text-emerald-200" : "font-medium text-slate-500"}>{connections?.ollamaConfigured ? "Available" : "Not configured"}</span></div></div></section><section className="rounded-[1.5rem] border border-amber-300/15 bg-amber-300/[0.04] p-5"><h2 className="text-sm font-semibold text-amber-100">Boundary</h2><p className="mt-2 text-xs leading-5 text-slate-400">The independent failsafe is outside Luna’s trust boundary. This dashboard shows status only; Luna cannot read, edit, disable, or rewrite it.</p></section></aside>
    </div>
  </div>;
}
