...
export default function LunaFoundation() {
  ...
  const update = trpc.knowledge.cognitive.foundation.update.useMutation({ onSuccess: () => { void utils.knowledge.cognitive.foundation.get.invalidate(); } });
  ...
  return <div className="senota-page mx-auto flex w-full max-w-6xl flex-col gap-6">
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_300px]">
      <section className="space-y-6 rounded-[1.5rem] border border-white/10 bg-card/70 p-5 sm:p-7">
        ...
        <div className="flex items-center justify-between gap-3"><p className="text-xs text-slate-500">Changes are saved to the authenticated Knowledge Space owner’s Luna cognitive state.</p><button onClick={save} disabled={isLoading || update.isPending} className="inline-flex items-center gap-2 rounded-xl bg-cyan-300 px-4 py-2.5 text-sm font-semibold text-slate-950 transition hover:bg-cyan-200 disabled:opacity-50"><Save className="size-4" /> {update.isPending ? "Saving…" : "Save foundation"}</button></div>{update.error ? <p className="text-sm text-rose-300">{update.error.message}</p> : null}
      </section>
