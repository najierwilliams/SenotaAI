...
interface AnatomicalInspectorProps {
  ...
  onScaleChange: (scale: BrainScale) => void;
}
    ...
    <aside className="absolute right-4 top-4 z-10 w-80 max-w-[calc(100%-2rem)] rounded-xl border border-white/10 bg-black/75 p-5 text-white shadow-2xl backdrop-blur-xl">
      ...
      <div className="mt-5 rounded-lg border border-white/10 bg-white/5 p-3">
        ...
        <div className="mt-2 grid grid-cols-2 gap-1">
          {BRAIN_SCALE_OPTIONS.map((option) => {
            const available = isBrainScaleAvailable(
              structure,
        ...
        <p className="mt-2 text-[9px] leading-relaxed text-white/30">
          Additional scale datasets can be registered
          without replacing the existing macro model.
        </p>
      </div>
      ...
      <div className="mt-5 rounded-lg border border-white/10 bg-white/5 p-3">
        ...
        <p className="mt-2 text-[9px] leading-relaxed text-white/30">
          Tissue through molecular views are scale targets ready for future multi-resolution datasets.
        </p>
      </div>
      ...
      <div className="mt-3">
        <button
          type="button"
          onClick={onToggleCrossSectionMode}
          className="w-full rounded-lg border border-white/10 bg-white/10 px-3 py-2 text-xs font-medium text-white transition hover:bg-white/15"
        >
          {crossSectionMode
            ? "Exit cross-section"
      ...
      <div className="mt-5 border-t border-white/10 pt-4">
        ...
        <p className="mt-2 text-xs leading-relaxed text-white/55">
          This structure is identified from the
          anatomical mesh contained in the Luna
          brain model.
        </p>
      </div>
    </aside>
