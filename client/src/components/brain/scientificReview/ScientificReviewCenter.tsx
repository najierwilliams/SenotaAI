...
export default function ScientificReviewCenter({ records, loading, summary, effectiveStatus, setDecision, resetToReview, structures, onSelectStructure, focusStructureId }: ScientificReviewCenterProps) {
  ...
  const displayed = useMemo(() => records.filter((record) => {
    ...
    if (filter === "high-confidence") return record.reviewStatus !== "unmapped" && !isComposite(record);
    if (filter === "ambiguous") return record.reviewStatus !== "unmapped" && isComposite(record);
  ...
  const authoritativeApproval = selected?.reviewStatus === "approved";

  return <aside className={summary.requiresReview > 0 ? "flex h-full w-full flex-col overflow-hidden rounded-xl border border-red-400/20 bg-black/75 text-white shadow-2xl backdrop-blur-xl" : "flex h-full w-full flex-col overflow-hidden rounded-xl border border-emerald-400/20 bg-black/75 text-white shadow-2xl backdrop-blur-xl"} aria-label="Scientific Review Center">
    <header className="border-b border-white/10 px-4 py-3">
      <div className="flex items-start justify-between gap-3"><div><div className={summary.requiresReview > 0 ? "text-[10px] font-semibold uppercase tracking-[0.18em] text-red-200" : "text-[10px] font-semibold uppercase tracking-[0.18em] text-emerald-200"}>Scientific Review</div><div className="mt-1 text-sm font-semibold">Human anatomical identity review</div><p className="mt-1 text-[10px] leading-relaxed text-white/45">Approval accepts anatomical identity only. It does not establish Luna → MNI, Luna → Julich, a coordinate, or nanobot capability.</p></div><div className={summary.requiresReview > 0 ? "rounded-md border border-red-300/35 bg-red-500/10 px-2 py-1 text-right" : "rounded-md border border-emerald-300/35 bg-emerald-500/10 px-2 py-1 text-right"}><div className={summary.requiresReview > 0 ? "text-lg font-semibold text-red-100" : "text-lg font-semibold text-emerald-100"}>{summary.requiresReview}</div><div className={summary.requiresReview > 0 ? "text-[8px] uppercase tracking-[0.14em] text-red-200/70" : "text-[8px] uppercase tracking-[0.14em] text-emerald-200/70"}>{summary.requiresReview > 0 ? "Remaining" : "Complete"}</div></div></div>
