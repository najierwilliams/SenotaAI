...
export default function ScientificReviewCenter({ records, loading, summary, effectiveStatus, setDecision, resetToReview, structures, onSelectStructure, focusStructureId }: ScientificReviewCenterProps) {
  ...
  return <aside className="flex h-full w-full flex-col overflow-hidden rounded-xl border border-red-400/20 bg-black/75 text-white shadow-2xl backdrop-blur-xl" aria-label="Scientific Review Center">
    ...
    <div className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(270px,0.9fr)]"><div className="min-h-0 overflow-y-auto p-2"><div className="mb-2 text-[9px] uppercase tracking-[0.15em] text-white/35">{filter.replace(/-/g, " ")} · {displayed.length} records</div>{loading ? <div className="p-4 text-xs text-white/40">Loading review evidence…</div> : displayed.map((record) => { const status = effectiveStatus(record); const composite = isComposite(record); return <button key={record.lunaStructureId} type="button" onClick={() => selectRecord(record)} className={selectedId === record.lunaStructureId ? "mb-1 w-full rounded-md border border-red-300/35 bg-red-500/10 p-3 text-left" : "mb-1 w-full rounded-md border border-white/8 bg-white/[0.025] p-3 text-left hover:bg-white/[0.06]"}><div className="flex items-center justify-between gap-2"><span className="truncate text-xs font-medium text-white/85">{record.lunaStructureName}</span><span className={status === "REQUIRES_REVIEW" ? "rounded border border-red-300/35 px-1.5 py-0.5 text-[8px] font-semibold text-red-200" : status === "REJECTED" ? "rounded border border-red-400/50 px-1.5 py-0.5 text-[8px] font-semibold text-red-100" : "rounded border border-emerald-300/25 px-1.5 py-0.5 text-[8px] font-semibold text-emerald-100"}>{status.replace(/_/g, " ")}</span></div><div className="mt-1 text-[9px] text-white/40">{record.uberonId ?? "No exact source-supported identity"} · {composite ? "Ambiguous/composite" : "High-confidence direct identity"}</div></button>; })}</div>
