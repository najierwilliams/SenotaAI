...
const decisionRow = { id: ids.decision, npc_id: "luna001", trigger_event_id: ids.event, trigger: "A test event was interpreted.", candidate_options: [], chosen_option: { action: "reflect", intention: "Compare the new information with current evidence.", rationale: "Reflection preserves uncertainty.", expectedOutcome: "A bounded hypothesis is preserved.", goalAlignment: 0.8, preferenceAlignment: 0, expectedValue: 0.6, uncertainty: 0.3, resourceCost: 0.1 }, score_breakdown: { total: 0.4, durableGoalContribution: 0.3 }, uncertainty: 0.3, intention: "Compare the new information with current evidence.", rationale: "Reflection preserves uncertainty.", safety_result: "internal-only", decision_mode: "mixed-self-directed", communication_intent: "reflect-briefly", ownership_summary: { goal: 0.4, counterfactual: 0.3, "user-direction": 0.3 }, status: "selected", created_at: now, updated_at: now };
const emptySnapshot = { state: { npcId: "luna001", mode: "active" as const, currentIntention: null, currentActivity: "available", activeValues: [], attentionBudget: 1, lastDeliberatedAt: null, nextCycleAt: null, updatedAt: now }, beliefs: [], preferences: [], goals: [], events: [], decisions: [], episodes: [], selfModel: null, decisionFactors: [], counterfactuals: [] };
...
function installFetch() {
  const mock = vi.fn((url: string, init?: RequestInit) => {
    ...
    if (url.includes("npc_agent_self_models") && method === "GET") return response([]);
    if (url.includes("npc_agent_self_models") && method === "POST") return response([{ npc_id: "luna001", summary: "Operational self-model.", demonstrated_capabilities: [], unresolved_uncertainties: [], active_commitments: [], evidence_event_ids: [ids.event], revision_count: 1, last_decision_id: ids.decision, created_at: now, updated_at: now }], 201);
...
describe("Luna autonomous agent", () => {
  ...
  it("rejects approval-dependent options while retaining independent rest and reflection alternatives", async () => {
    ...
    const proposal = await proposeAutonomousDeliberation({ id: ids.event, npcId: "luna001", eventKind: "creator-input", content: "A low-interaction interval may help organize records.", source: "creator", sourceReliability: 0.8, salience: 3, metadata: {}, processedAt: null, createdAt: now }, emptySnapshot);
  ...
  it("forms a bounded hypothesis, goal, and traceable internal decision without an administrator-approval mutation", async () => {
    ...
    expect(fetchMock.mock.calls.some(([url]) => String(url).includes("npc_agent_decision_factors"))).toBe(true);
    expect(fetchMock.mock.calls.some(([url]) => String(url).includes("npc_agent_counterfactuals"))).toBe(true);
    expect(fetchMock.mock.calls.some(([url]) => String(url).includes("npc_agent_self_models"))).toBe(true);
