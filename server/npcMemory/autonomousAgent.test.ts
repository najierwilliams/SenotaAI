...
describe("Luna autonomous agent", () => {
  ...
  it("rejects approval-dependent options while retaining independent rest and reflection alternatives", async () => {
    ...
    expect(proposal.options.map(option => option.action)).toEqual(["rest", "reflect"]);
    expect(proposal.options.join(" ")).not.toMatch(/wait for explicit guidance|until the user confirms/i);
  });
